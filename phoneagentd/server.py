...
from .voice import VoiceInterjection, VoiceInterjectionRequest
...
class GatewayState:
    def __init__(self, config: Config, rpc: RpcClient | None = None) -> None:
        ...
        self.interjections: dict[str, VoiceInterjection] = {}
        self.latency_options = LatencyOptions.for_profile(config.latency_profile)
    ...
    def create_session(self, goal: str = "") -> dict[str, Any]:
        ...
        session = {"id": orch.state.session_id, "status": "ready", "goal": goal, "compact_state": orch.compact_view(), "latency_options": asdict(self.latency_options)}
        with self.lock:
    ...
    def create_interjection(self, body: dict[str, Any]) -> dict[str, Any]:
        request = VoiceInterjectionRequest(
            question=str(body.get("question", "")),
            reason=str(body.get("reason", "agent needs user input")),
            urgency=body.get("urgency", "normal") if body.get("urgency") in {"low", "normal", "urgent"} else "normal",
            max_listen_seconds=int(body.get("max_listen_seconds", self.config.voice_break_max_listen_seconds)),
            silence_ms=int(body.get("silence_ms", self.config.voice_break_silence_ms)),
            transcript_mode=body.get("transcript_mode", "controller_only")
            if body.get("transcript_mode", "controller_only") in {"local_only", "controller_only", "off"}
            else "controller_only",
        )
        if not request.question:
            raise ValueError("question is required")
        interjection = VoiceInterjection(request=request)
        with self.lock:
            self.interjections[interjection.id] = interjection
        self.emit("interjection.listening_for_break", interjection.to_dict())
        return interjection.to_dict()

    def complete_interjection(self, interjection_id: str, delivered: bool) -> dict[str, Any]:
        with self.lock:
            interjection = self.interjections.get(interjection_id)
        if not interjection:
            raise KeyError("interjection not found")
        interjection.complete(delivered)
        self.emit("interjection.completed", interjection.to_dict())
        return interjection.to_dict()
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            try:
                if path == "/health":
                    ...
                    _send(self, HTTPStatus.OK, {"status": "ready", "bridge": f"{state.config.rpc_host}:{state.config.rpc_port}", "latency_options": asdict(state.latency_options), "latency_descriptions": state.latency_options.descriptions(), "voice_break_mode": state.config.voice_break_mode})
                ...
                elif path.startswith("/v1/interjections/"):
                    interjection_id = path.rsplit("/", 1)[-1]
                    interjection = state.interjections.get(interjection_id)
                    _send(self, HTTPStatus.OK if interjection else HTTPStatus.NOT_FOUND, interjection.to_dict() if interjection else {"error": "interjection not found"})
                else:
        ...
        def do_POST(self) -> None:  # noqa: N802
            ...
            try:
                ...
                elif path == "/v1/interjections":
                    _send(self, HTTPStatus.CREATED, state.create_interjection(body))
                ...
                elif path.startswith("/v1/interjections/") and path.endswith("/complete"):
                    interjection_id = path.split("/")[3]
                    _send(self, HTTPStatus.OK, state.complete_interjection(interjection_id, bool(body.get("delivered", True))))
