...
- `POST /v1/commands`
- `GET /v1/events`
- `POST /v1/interjections`
- `GET /v1/interjections/{interjection_id}`
- `POST /v1/interjections/{interjection_id}/complete`
...
PHONEAGENT_LATENCY_PROFILE=balanced
PHONEAGENT_VOICE_BREAK_MODE=true
PHONEAGENT_VOICE_BREAK_MAX_LISTEN_SECONDS=45
PHONEAGENT_VOICE_BREAK_SILENCE_MS=900
```
...
## Low-latency modes and trade-offs

The gateway now exposes latency options in `GET /v1/status` so the Mac/iPhone
controller can show exactly what each speed toggle does. Supported profiles are:

- `balanced`: safest default; prefetch accessibility trees, verify after actions,
  and request screenshots only when needed.
- `fast`: prewarms the local model and reduces verification delay while still
  re-observing after UI changes.
- `ultra_low`: prefetches accessibility tree and screenshot thumbnails up front
  and permits optimistic low-risk navigation. This is fastest, but uses more
  battery and captures more local visual context.

Screenshot prefetching should stay local by default. Do not send raw screenshots
to cloud models unless the user explicitly enables that in the controller.

## Natural-break voice interjection portal

When the agent needs to ask a question or confirm a risky action, it can create a
voice interjection request with `POST /v1/interjections`. The intended iPhone
controller behavior is:

1. Open the microphone only for the requested interjection window.
2. Run speech activity detection / speech-to-text on-device or through a local
   STT service.
3. If someone is talking, wait for the configured silence threshold (default
   `900 ms`).
4. Interject with the agent question.
5. Call `/complete` and immediately shut the microphone path off to save battery.

This is a presence/natural-pause detector, not an always-on recorder. Cloud STT
should remain off unless the user explicitly opts in.

## macOS host controller app

The `client/` directory now contains a lightweight Electron macOS shell for the
PhoneAgent gateway. It includes gateway settings, bearer-token entry, connection
test, quick actions, latency/privacy toggles with trade-off descriptions, and a
voice interjection portal panel. The packaged Electron app loads the committed `client/renderer/` bundle directly. On macOS, package an app/DMG with:
